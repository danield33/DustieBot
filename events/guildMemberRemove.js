...
bot.on('guildMemberRemove', async member => {
    if (await Guild.get(member.guild.id, "modules.bye.toggle")) {
        let wChannel = member.guild.channels.get(await Guild.get(member.guild.id, "modules.bye.channel"))
        if (!wChannel) return
        let format = await Guild.get(member.guild.id, "modules.bye.format")
        /*
            Image WELCOME
        */
        if (format === 0) {
            let sett = await Guild.get(member.guild.id, "modules.bye")
            ...
            let array = await Guild.get(member.guild.id, "modules.bye.randmsgs")
            if (!array) {
                ...
                let sMessage = await Guild.get(member.guild.id, "modules.bye.text")
